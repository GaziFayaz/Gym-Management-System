...
export class UserController {
	...
	static async createUser(
		...
		try {
			...
			if (userData.role === Role.TRAINER) {
				...
				if (!currentUser || currentUser.role !== Role.ADMIN) {
					const response = ResponseUtils.forbidden(
						"Only admins can create trainers"
					);
			...
			const user = await UserService.createUser(userData, currentUser?.userId);
	...
	static async createAdmin(
		...
		try {
			...
			const response = ResponseUtils.error(500, "Failed to create admin");
			res.status(response.statusCode).json(response);
		}
	}
	...
	static async getMyTrainers(
		...
		try {
			if (!req.user || req.user.role !== Role.ADMIN) {
				const response = ResponseUtils.forbidden(
					"Only admins can view their trainers"
				);
				res.status(response.statusCode).json(response);
				return;
			}
			...
			const response = ResponseUtils.success(
				200,
				"My trainers retrieved successfully",
				trainers
			);
			res.status(response.statusCode).json(response);
		} catch (error) {
